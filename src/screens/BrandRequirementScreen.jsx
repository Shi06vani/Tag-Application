  ...
  return (
    ...
    {loading ? (
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View className="bg-white p-4 mb-3 rounded-2xl shadow-lg border-l-4 border-accent">
            <TouchableOpacity onPress={() => navigation.navigate("Brand-details", { brandid: item?.brandId?._id })}>
              <Text className="text-xl font-bold text-primary">{item.title}</Text>
            </TouchableOpacity>
            <Text className="text-gray-600 mt-2">{item.description}</Text>
            <Text className="text-primary font-semibold mt-2">category: {item.category}</Text>
